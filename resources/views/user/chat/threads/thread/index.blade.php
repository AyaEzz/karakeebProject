                ...
                <div class="panel-body">
                    ...
                    @if (count($users))
                        @foreach ($users as $user)
                            <div><a href="{{ route('user.chat.threads.thread.messages.index', $user) }}">View {{ '@' . $user->firstname }} messages {!! Auth::user()->hasUnreadMessagesFromSender($user) ? '<span class="badge">' . Auth::user()->unreadMessageCountForSender($user). '</span>' : '' !!}</a></div>
                        @endforeach
                    @else
                ...
                <div class="panel-body">
                    ...
                    <form action="{{ route('user.chat.threads.create') }}" method="post">
                        <div class="form-group{{ $errors->has('recipient') ? ' has-error' : '' }}">
                            ...
                            <input type="text" name="recipient" id="recipient" class="form-control" value="{{ (old('recipient') ? old('recipient') : '@' . Request::get('recipient') ) }}" placeholder="@firstname">
